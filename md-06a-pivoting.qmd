...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below. The tidyverse package includes several packages that are often used for cleaning, transforming, handling, visualizing data.
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format? it is better but I would reorder columns and add a caption etc.
...
```{r}
ggplot(waste_data_long_mean, aes(x = mean_percent, y = waste_category, fill = income_cat)) +
  ...
  ylab("Waste category") +
  theme_minimal() +
  ggthemes::scale_fill_colorblind(name = "Income category") +
